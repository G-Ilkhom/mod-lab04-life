...
namespace Life.Tests
{
    public class LifeBoardTests
    {
        ...
        {
            var b = CreateBoard(5, 5);
            b.Grid[1, 1].FluxState = true;
            b.Grid[1, 2].FluxState = true;
            b.ProgressFrame();
            Assert.Empty(GetAlive(b));
        }
        ...
        [Fact]
        public void ThreeVertical_Periodic_ReturnsToInitialAfterTwoFrames()
        {
            var b = CreateBoard(5, 5);
        ...
        public void Block_StaysStable()
        {
            ...
            foreach (var c in init) b.Grid[c.Item1, c.Item2].FluxState = true;
            b.ProgressFrame();
            AssertCellsEqual(init, GetAlive(b));
        }
        ...
        [Fact]
        public void Glider_MovesDiagonallyAfterFourFrames()
        {
            var b = CreateBoard(5, 5);
            var glider = new[] { (1, 0), (2, 1), (0, 2), (1, 2), (2, 2) };
            foreach (var c in glider) b.Grid[c.Item1, c.Item2].FluxState = true;
            for (int i = 0; i < 4; i++) b.ProgressFrame();
        ...
        public void WrapAround_DetectsNeighborsAcrossEdges()
        {
            ...
            b.Grid[0, 0].FluxState = true;
            b.Grid[2, 0].FluxState = true;
            b.ProgressFrame();
            Assert.Empty(GetAlive(b));
        }
        ...
        public void Analyze_DisconnectedClusters()
        {
            ...
            foreach (var c in clusterA.Concat(clusterB)) b.Grid[c.Item1, c.Item2].FluxState = true;
            var (alive, clusters) = b.Analyze();
        ...
        public void Analyze_MultipleClustersCount()
        {
            ...
            foreach (var c in coords) b.Grid[c.Item1, c.Item2].FluxState = true;
        ...
        public void ToadOscillator_TogglesBetweenPhases()
        {
            ...
            foreach (var c in phase1) b.Grid[c.Item1, c.Item2].FluxState = true;
            b.ProgressFrame();
        ...
        public void Blinker_PeriodTwo_ReturnsToInitial()
        {
            ...
            foreach (var c in initial) b.Grid[c.Item1, c.Item2].FluxState = true;
            b.ProgressFrame();
            b.ProgressFrame();
